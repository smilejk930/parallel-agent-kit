...
- **Sub-commands:**
	- **`list`**:
		- **Description:** Lists all discovered agents, including built-in, local, and remote agents.
				- **Usage:** `/agents list`
		- **`reload`** (alias: `refresh`):
		- **Description:** Rescans agent directories (`~/.gemini/agents` and `.gemini/agents`) and reloads the registry.
				- **Usage:** `/agents reload`
		- **`enable`**:
		...
		- **`config`**:
		- **Description:** Opens a configuration dialog for the specified agent to adjust its model, temperature, or execution limits.
...
- **Description:** Alias for `/resume`. Both commands now expose the same session browser action and checkpoint subcommands.
...
- **Sub-commands:**
	- **`debug`**
		...
		- **`share [filename]`**
		- **Description:** Writes the current conversation to a provided Markdown or JSON file. If no filename is provided, then the CLI will generate one.
...
- **Sub-commands:**
	- **`reload`**:
		- **Description:** Reload custom command definitions from all sources (user-level `~/.gemini/commands/`, project-level `<project>/.gemini/commands/`, MCP prompts, and extensions). Use this to pick up new or modified `.toml` files without restarting the CLI.
...
- **Description:** Manage extensions. See [Gemini CLI Extensions](https://geminicli.com/docs/extensions).
- **Sub-commands:**
	- **`config`**:
...
- **Sub-commands:**
	- **`auth`**:
		...
		- **`reload`**:
		- **Description:** Reloads all MCP servers and re-discovers their available tools.
		- **`schema`**:
...
- **Sub-commands:**
	- **`add`**:
		...
		- **Note:** For more details on how `GEMINI.md` files contribute to hierarchical memory, see the [CLI Configuration documentation](https://geminicli.com/docs/reference/configuration).
...
- **Description:** Switch to Plan Mode (read-only) and view the current plan if one has been generated.
	- **Note:** This feature is enabled by default. It can be disabled via the `general.plan.enabled` setting in your configuration.
- **Sub-commands:**
	- **`copy`**:
...
- **Note:** Only available if checkpointing is configured via [settings](https://geminicli.com/docs/reference/configuration). See [Checkpointing documentation](https://geminicli.com/docs/cli/checkpointing) for more details.
...
- **Features:**
	- **Auto sessions:** Run `/resume` to open the interactive session browser for automatically saved conversations.
		- **Chat checkpoints:** Use checkpoint subcommands directly (`/resume save`, `/resume resume`, etc.).
...
- **Sub-commands:**
	- **`list`**
		...
		- **`debug`**
		- **Description:** Export the most recent API request as JSON payload (nightly builds).
		- **Compatibility alias:** `/resume checkpoints ...` is still accepted for the same checkpoint commands.
...
- **Sub-commands:**
	- **`disable <name>`**:
		...
		- **`list`**:
		- **Description:** List all discovered skills and their current status (enabled/disabled).
		- **`reload`**:
		- **Description:** Refresh the list of discovered skills from all tiers (workspace, user, and extensions).
...
- **Sub-commands:**
	- **`session`**:
		- **Description:** Show session-specific usage statistics, including duration, tool calls, and performance metrics. This is the default view.
		- **`model`**:
		- **Description:** Show model-specific usage statistics, including token counts and quota information.
		- **`tools`**:
...
- **Features:**
	- **Count support:** Prefix commands with numbers (for example, `3h`, `5w`, `10G`)
...
Custom commands allow you to create personalized shortcuts for your most-used prompts. For detailed instructions on how to create, manage, and use them, see the dedicated [Custom Commands documentation](https://geminicli.com/docs/cli/custom-commands).
...
- **`@<path_to_file_or_directory>`**
	- **Description:** Inject the content of the specified file or files into your current prompt. This is useful for asking questions about specific code, text, or collections of files.
		...
		- If a path to a single file is provided, the content of that file is read.
				...
				- Spaces in paths should be escaped with a backslash (for example, `@My\ Documents/file.txt`).
...
- If the `read_many_files` tool encounters an error (for example, permission issues), this will also be reported.
...
- **Environment variable:** When a command is executed via `!` or in shell mode, the `GEMINI_CLI=1` environment variable is set in the subprocess’s environment. This allows scripts or tools to detect if they are being run from within Gemini CLI.