...
Hooks are defined in `settings.json` within the `hooks` object. Each event (for example, `BeforeTool`) contains an array of **hook definitions**.
...
- **Built-in Tools**: You can match any built-in tool (for example, `read_file`, `run_shell_command`). See the [Tools Reference](https://geminicli.com/docs/reference/tools) for a full list of available tool names.
- **MCP Tools**: Tools from MCP servers follow the naming pattern `mcp_<server_name>_<tool_name>`.
- **Regex Support**: Matchers support regular expressions (for example, `matcher: "read_.*"` matches all file reading tools).
...
- **Relevant Output Fields**:
	- `decision`: Set to `"deny"` to hide the real tool output from the agent.
		...
		- `hookSpecificOutput.tailToolCallRequest`: (`{ name: string, args: object }`) A request to execute another tool immediately after this one. The result of this “tail call” will replace the original tool’s response. Ideal for programmatic tool routing.
		- `continue`: Set to `false` to **kill the entire agent loop** immediately.
...
- **Relevant Output Fields**:
	- `hookSpecificOutput.llm_request`: An object that **overrides** parts of the outgoing request (for example, changing models or temperature).
...
- **Input Fields**:
	- `notification_type`: (`"ToolPermission"`)
		...
		- `details`: JSON object with alert-specific metadata (for example, tool name, file path).
- **Relevant Output Fields**:
